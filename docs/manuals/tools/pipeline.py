...
STATES = [
    ...
    ("throw", "_ride", "js:var st=document.createElement('style');st.textContent='*{transition:none!important;animation:none!important}';document.head.appendChild(st);window.seedMacro(0,'End Of Build');window.recallMacro(0);engineFb=0.83;engineBandHz=759;engineDelayMs=316.9;engineBpm=142;state.throw=0.2;state.rideBars=2;ridePhase=2;rideEff=0.62;renderRide();renderKnob();renderTaps();renderEngine()", None),
