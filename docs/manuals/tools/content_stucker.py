def manual(G):
    ...
    m = {
        ...
        "title_html": "RONE <i>Stucker</i>", "accent": "#9D6BFF", "version": "1.2",
...
{legend([
 ...
 ("GO", "starts the ride from the downbeat of the bar you press it in; it lands N bars later (MIDI C2)"),
 ...
 ("ADVANCED", "opens the panel with SMOOTH, RANGE, LOW CUT, LENGTH, KEY, ON THE DROP and CURVE"),
...
{ctl("RIDE and GO", "OFF &middot; 1 BAR &middot; 2 &middot; 4 &middot; 8<br>default OFF<br>GO: momentary, automatable, MIDI C2", "<p>The knob rides itself, on the bar. Pick a length and press <strong>GO</strong> anywhere in the bar the rise starts in: the ride is measured from that bar's downbeat, so the STUCK climbs from wherever the knob is to 100 % over that many bars and lands exactly N bars after the bar you marked - where it snaps straight back to the knob. The press is the mark; nothing waits for a next bar. With the knob at OFF that is the release, exactly on the drop, with whatever ON THE DROP says. The big knob follows on screen.</p><p>It is the snare roll-up automation of every build, without drawing it, and it cannot be late. The bar comes from the host's song position; in the standalone app, or while the transport is stopped, the clock free-runs at the tempo. Setting RIDE to OFF cancels a ride in progress.</p>", "GO is also a parameter (Ride Go): automate one step at the top of the last phrase, or send MIDI C2 from a pad. The CURVE row in ADVANCED sets the shape.")}
...
{ctl("KEY", "OFF, C .. B<br>default OFF", "<p>A scream at an arbitrary pitch fights the bass; on the root it is part of the music. Choose the song's root and the top of the knob lands exactly on it - in the octave nearest to where it would have landed anyway at this tempo and RANGE - and the LANDS chip on the main view shows the note. On the way up, once the loop is at audio rate, the pitch steps in semitones instead of sliding, each step glided by SMOOTH, so the roll-up becomes a run up to the root on the downbeat.</p>", "Psytrance in E: KEY E, RANGE 8. The scream resolves on E on the drop, over an E bass.")}
...
{recipe("The one-bar build", "Any drop, any genre",
 steps([
  ...
  "Set RIDE to 1 BAR. In the bar before the drop press GO (or send MIDI C2, or automate one step of Ride Go).",
  "The knob climbs by itself from that bar's downbeat and releases exactly on the downbeat of the drop. Nothing to draw.",
...
{recipe("In key", "Psytrance, any melodic drop",
 steps([
  "Open ADVANCED. KEY = the song's root.",
  ...
  "Ride or automate STUCK to 100 %: the run up steps in semitones and lands on the root on the downbeat.",
 ]))}
...
"""})
    ...
    S.append(G["support_section"](m, [
        ...
        ("The ride landed early or late", "GO measures from the downbeat of the bar you pressed it in. Press it inside the bar the rise should start in; a press just after a downbeat and one just before the next both count for the same bar."),
